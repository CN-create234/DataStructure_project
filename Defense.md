...
> 每个点初始指向自己，查找时递归压缩路径，合并时按秩把矮树接高树下，近乎常数时间判断两个顶点是否已连通，为 Kruskal 提供成环检测。

### Dijkstra 最短路径

> 从起点开始，用优先队列每次取距离最小的顶点向外扩散，松弛所有邻接边，直到所有可达顶点处理完毕；同时支持距离/时间两种权重、开放时间过滤，并在多条最短路径时自动选择字典序最小的那条。

### 连通分量

> 从每个未访问顶点出发 BFS 扩散，仅沿 open 边走，访问过的全部标记，统计每波扩散的人数，最后降序输出各分量大小。

### 必经点路径

> 将起点、必经点序列、终点按顺序切分成多段，每段独立调用 Dijkstra，拼接时跳过段间重复节点（前段终点即后段起点），任一段走不通则整体宣告不可达。

### 最小生成树

> 收集所有 open 边按距离升序排列，用 DSU 从小到大尝试加入，若两端已连通则跳过（避免成环），直到选中 V-1 条边则连通，否则图本身不连通。

### 关键节点与关键边

> 先算原图连通分量数作为基准，逐个删除顶点/边后重算连通分量，比基准多则说明该点/边是网络命脉，结果按字典序排序输出。

### 分层图最短路

> 将原图复制 K+1 层，状态扩展为 (地点, 已用券数)，不用券在同一层内按原边权转移，用券则跳到下一层且边权变为 ceil(walk_time/3)，跑完 Dijkstra 后在所有层中找终点最小时间，回溯时收集用券边并排序输出。

##  数据结构设计
> 邻接表存储每个顶点的关联边ID，配合顶点哈希索引、边红黑树索引、类别哈希-集合索引，共四层索引让增删改查各自走最快通道，边池只存一份实现双向共享。
...
├───────────────────────────────────────────────────────────────┤
│  ① 顶点定位索引         ② 顶点数据池                            │
│  id_to_vid_             vertices_                             │
│  "P0001" → 0            [0] = {info, edge_ids: [0,2,5]}       │
│  "P0002" → 1            [1] = {info, edge_ids: [0,1,3]}       │
│  "P0003" → 2            [2] = {info, edge_ids: [1,4]}         │
├───────────────────────────────────────────────────────────────┤
│  ③ 边池                 ④ 边索引                               │
│  edges_                 edge_index_                           │
│  [0] = {P0001,P0002}    {"P0001","P0002"} → 0                 │
│  [1] = {P0002,P0003}    {"P0002","P0003"} → 1                 │
│  [2] = {P0001,P0004}    {"P0001","P0004"} → 2                 │
├───────────────────────────────────────────────────────────────│
│  ⑤ 类别索引                                                    │
│  category_index_                                              │
│  "Teaching" → {"P0001","P0002"}                               │
│  "Dining"   → {"P0003"}                                       │
└───────────────────────────────────────────────────────────────┘
```
